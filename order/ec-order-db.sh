...
database_name="ecorderdb"
 
create_database="create database ecorderdb"
...
) then raise exception 'Table[orders] is journaled. Updates to primary key column[id] are not supported as this would make it impossible to follow the history of this row in the journal table[ec_orders_processing_queue]';
